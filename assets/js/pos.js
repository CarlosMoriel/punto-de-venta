	...
	$(document).ready(function () {
		...
		function loadProducts() {
			$.get(api + "inventory/products", function (data) {
				...
				$("#parent").text("");
				$("#categories").html(
					`<button type="button" id="all" class="btn btn-categories btn-white waves-effect waves-light">Todos</button> `
				);
					...
					let item_info = `<div class="col-lg-2 box ${item.category}"
						...
						item.stock
                            ...
                            <div id="image"><img src="${
								item.img == "" ? "./assets/images/default.jpg" : img_path + item.img
		...
		function loadCustomers() {
			$.get(api + "customers/all", function (customers) {
				$("#customer").html(
					`<option value="0" selected="selected">Cliente sin cita previa</option>`
				);
		...
		$.fn.addToCart = function (id, count, stock) {
			if (stock == 1) {
				if (count > 0) {
					...
					Swal.fire("Agotado", "Este artículo no está disponible actualmente", "info");
				}
			} else {
				$.get(api + "inventory/product/" + id, function (data) {
					$(this).addProductToCart(data);
		...
		$.fn.qtIncrement = function (i) {
			...
			if (product[0].stock == 1) {
				if (item.quantity < product[0].quantity) {
					...
					Swal.fire(
						"¡Se acabaron los stock!",
						"Ya ha añadido todas las existencias disponibles.",
						"info"
					);
				}
			} else {
		...
		$.fn.cancelOrder = function () {
			if (cart.length > 0) {
				Swal.fire({
					title: "Esta seguro?",
					text: "Está a punto de eliminar todos los artículos de la cesta.",
					icon: "warning",
					...
					if (result.value) {
						...
						Swal.fire("¡Despejado!", "Se han retirado todos los artículos.", "success");
					}
				});
			}
		};
		...
		$("#payButton").on("click", function () {
			if (cart.length != 0) {
				...
				Swal.fire("Oops!", "No hay nada que pagar.", "warning");
			}
		});
		...
		$("#hold").on("click", function () {
			if (cart.length != 0) {
				...
				Swal.fire("Oops!", "No hay nada que sujetar.", "warning");
			}
		});

		function printJobComplete() {
			alert("trabajo de impresión finalizado");
		}
		...
		$.fn.submitDueOrder = function (status) {
			...
			let change =
				$("#change").text() == "" ? "" : parseFloat($("#change").text()).toFixed(2);
			...
			if (status == 0) {
				if ($("#customer").val() == 0 && $("#refNumber").val() == "") {
					Swal.fire(
						...
						"¡Debe seleccionar un cliente r <br> o introducir una referencia!",
						"warning"
					);
        ...
        ${
            ...
            ${settings.tax != "" ? "Núm de IVA: " + settings.tax + "<br>" : ""} 
        </p>
        <hr>
        <left>
            <p>
            Numero Orden : ${orderNumber} <br>
            Num Ref : ${refNumber == "" ? orderNumber : refNumber} <br>
            Cliente : ${customer == 0 ? "Cliente sin cita previa" : customer.name} <br>
		...
		$.fn.deleteOrder = function (index, type) {
			...
			Swal.fire({
				title: "¿Borrar orden?",
				text: "Esto eliminará el pedido. ¿Está seguro de que desea borrar?",
				icon: "warning",
				showCancelButton: true,
				confirmButtonColor: "#3085d6",
				cancelButtonColor: "#d33",
				...
				if (result.value) {
					$.ajax({
						...
						success: function (data) {
							...
							Swal.fire("Borrado!", "Ha borrado el pedido.", "success");
						},
						error: function (data) {
							$(".loading").hide();
						},
		...
		$("#saveCustomer").on("submit", function (e) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					Swal.fire("Cliente añadido", "¡Cliente añadido correctamente!", "success");
					$("#customer option:selected").removeAttr("selected");
					$("#customer").append(
						$("<option>", {
							text: custData.name,
				...
				error: function (data) {
					$("#newCustomer").modal("hide");
					Swal.fire("Error", "Algo ha ido mal, por favor inténtelo de nuevo", "error");
				},
			});
		});
		...
		$("#confirmPayment").on("click", function () {
			if ($("#payment").val() == "") {
				Swal.fire("Nope!", "Por favor, introduzca el importe pagado.", "warning");
			} else {
		...
		function loadProductList() {
            ...
            <td><img style="max-height: 50px; max-width: 50px; border: 1px solid #ddd;" src="${
				product.img == "" ? "./assets/images/default.jpg" : img_path + product.img
			}" id="product_img"></td>
            <td>${product.name}</td>
		...
		$("#settings_form").on("submit", function (e) {
			...
			if (formData.percentage != "" && !$.isNumeric(formData.percentage)) {
				Swal.fire("Oops!", "Asegúrese de que el valor fiscal es un número", "warning");
			} else {
		...
		$("#net_settings_form").on("submit", function (e) {
			...
			if (formData.till == 0 || formData.till == 1) {
				Swal.fire("Oops!", "Por favor, introduzca un número mayor que 1.", "warning");
			} else {
				if (isNumeric(formData.till)) {
					...
					Swal.fire("Oops!", "¡Hasta el número debe ser un número!", "warning");
				}
			}
		});
				...
				$.ajax({
					...
					success: function (data) {
						if (ownUserEdit) {
							...
							$("#Users").modal("show");
							Swal.fire("Ok!", "Datos de usuario guardados", "success");
						}
					},
					error: function (data) {
						console.log(data);
		...
		if (transactions.length > 0) {
			...
			transactions.forEach((trans, index) => {
				...
				transaction_list += `<tr>
                                ...
                                <td>${
									trans.change
										? settings.symbol + Math.abs(trans.change).toFixed(2)
										: ""
								}</td>
                                <td>${
									trans.paid == ""
...
function loadSoldProducts() {
	...
	sold.forEach((item, index) => {
		...
		sold_list += `<tr>
            ...
            <td>${item.qty}</td>
            <td>${
				product[0].stock == 1 ? (product.length > 0 ? product[0].quantity : "") : "N/A"
			}</td>
...
$.fn.viewTransaction = function (index) {
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td>${
				discount > 0
					? settings.symbol + parseFloat(allTransactions[index].discount).toFixed(2)
					: ""
			}</td>
        </tr>
